...
#import "QUYunManager.h"

#import <QiniuSDK.h>

@interface QUYunManager()

@property (nonatomic, strong) QNUploadManager* qnUploadMgr;

@end

@implementation QUYunManager


+ (instancetype)shareQNYunManager
{
    static QUYunManager* manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[QUYunManager alloc] init];
...
//    NSString *filePath = @"要上传的文件路径";
    ...
    [[QUYunManager shareQNYunManager].qnUploadMgr putData:data key:imgkey token:token complete:^(QNResponseInfo *info, NSString *key, NSDictionary *responseObj) {
        if(info.ok)
        {
            NSLog(@"请求成功");
        }
...
{
    [[QUYunManager shareQNYunManager].qnUploadMgr putFile:filePath key:imgkey token:token complete:^(QNResponseInfo *info, NSString *key, NSDictionary *responseObj) {
